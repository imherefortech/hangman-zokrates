...
async function deploy(framework) {
    ...
    const verifierDeployTx = await factory.deploy(verifierBytecode, ethers.utils.id(`hangman-${framework}-verifier-test-deploy`));
    ...
    const hangmanDeployTx = await factory.deploy(hangmanInitCode, ethers.utils.id(`hangman-${framework}-v1-test-deploy`));
